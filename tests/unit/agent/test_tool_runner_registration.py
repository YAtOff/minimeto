import pytest
import logging
from meto.agent.tool_runner import register_tool_handler, _TOOL_HANDLERS

def test_register_tool_handler_overwrite_warning(caplog):
    # Setup: ensure a tool exists
    def original_handler(_ctx, _params): return "original"
    def new_handler(_ctx, _params): return "new"
    ...
    try:
        ...
        assert "Overwriting existing tool handler with a different one: test_warning_tool" in caplog.text
